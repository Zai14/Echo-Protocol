...
void EchoMemoryReveal(EchoMemory *memory, float worldX, float worldY, float gameTime)
{
    ...
    memory->cells[memory->count].corrupted   = false;
    memory->count++;
}

/* ------------------------------------------------------------------ */
/*  Corrupt — mark a grid cell as corrupted by the Phantom            */
/* ------------------------------------------------------------------ */
...
bool EchoMemoryCorrupt(EchoMemory *memory, float worldX, float worldY)
{
    ...
    {
        if (memory->cells[i].cellX == cx && memory->cells[i].cellY == cy &&
            memory->cells[i].active)
        {
            memory->cells[i].corrupted = true;
            return true;
        }
    }
    return false;
...
float EchoMemoryGetCellColor(float age, bool corrupted, Color *outColor)
{
    ...
    if (age < ECHO_MEMORY_TRANSITION_TIME)
    {
        ...
        if (corrupted) {
            /* Corrupted full-vis: shifted toward magenta. */
            float t = age / ECHO_MEMORY_TRANSITION_TIME;
            outColor->r = (unsigned char)(200.0f * (1.0f - t) + 180.0f * t);
            outColor->g = (unsigned char)(100.0f * (1.0f - t) + 80.0f  * t);
            outColor->b = (unsigned char)(200.0f * (1.0f - t) + 180.0f * t);
        } else {
            /* Normal: Lerp from bright white (age=0) to wireframe blue (age=TRANSITION). */
            float t = age / ECHO_MEMORY_TRANSITION_TIME;
    ...
    if (corrupted)
    {
        /* Corrupted wireframe: purple/magenta with static-like variation.
         * Oscillates between magenta and purple for a glitchy feel. */
        float glitch = sinf(age * 8.0f) * 0.2f + 0.8f;
        outColor->r = (unsigned char)(180.0f * glitch);
        outColor->g = (unsigned char)(60.0f  * glitch);
        outColor->b = (unsigned char)(180.0f * glitch);
    }
    else if (decayRatio > 0.6f)
    {
        /* Normal degradation: in the last 40% of the fade,
